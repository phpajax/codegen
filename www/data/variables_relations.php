<?php
	$variables_relations = [ // content <- data
		...
		'integer^boolean' => ['{c} = (int) !{d}', '{c} = intval({d})'],
		'integer^string' => [
			'{c} = strlen({d})', '{c} = intval({d})', '{c} = array_sum(array_map(function($s){return ord($s);}, str_split({d})))', '{c} = ord({d})', '{c} = str_word_count({d})'
		],   
		'integer^array' => [
			'{c} = count({d})', '{c} = count(array_keys({d}))', '{c} = array_sum(array_values({d}))', '{c} = array_sum(array_map(function($s){return ord($s);}, {d}))'
		],
		'integer^object' => [
